...
export default async function handler(
  ...
  const { doctor, doctorID, specialty, day, time, patientName, patientPhone, reasonForVisit } = req.body;

  if (!doctor || !doctorID || !day || !time || !patientName || !patientPhone || !specialty || !reasonForVisit) {
  ...
  try {
    const response = await fetch(webhookUrl, {
      ...
      body: JSON.stringify({ doctor, doctorID, specialty, day, time, patientName, patientPhone, reasonForVisit }),
    });
